...
struct o_array *f_array_new(struct o_array *supplied, size_t size) {
	...
	if ((result = (struct o_array *)f_object_new(v_array_kind, sizeof(struct o_array), (struct o_object *)supplied))) {
		if ((result->content = (struct o_object **) calloc(size, sizeof(struct o_object *)))) {
			result->size = size;
...
int p_array_compare(struct o_object *object, struct o_object *other) {
	struct o_array *local_object = (struct o_array *)object, *local_other = (struct o_array *)other;
...
char *p_array_string(struct o_object *object, char *data, size_t size) {
	...
	size_t index;
	for (index = 0; index < local_object->size; index++) {
		next = (local_object->content[index])->s_delegate.m_string(local_object->content[index], pointer, size);
...
struct o_object *p_array_clone(struct o_object *object) {
	struct o_array *result = (struct o_array *)p_object_clone(object), *local_object = (struct o_array *)object;
	size_t index;
	if (local_object->size > 0)
		if ((result->content = (struct o_object **) calloc(1, local_object->size)))
			for (index = 0; index < local_object->size; index++)
